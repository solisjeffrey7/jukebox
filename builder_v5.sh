...
# ============================================================
# OFFLINE INSTALLER BUILDER v3.25
# ============================================================
#
# BASE: v3.24
#
...
# • Better stale repository handling
# • Clean partial DEB retry
...
# • Completed [OK] results remain above progress
# • Smart retry
# • Clean error UI
# • Detailed error.log
# • libacl bootstrap
...
# ============================================================
#
# Refresh only once per builder execution.
#
# Existing cache is NEVER deleted.
#
# ============================================================
...
refresh_apt_metadata() {
    ...
    show_progress \
        "Refreshing Termux package metadata"
...
download_apt_package() {
    ...
    if [ -z "$candidate" ] ||
       [ "$candidate" = "(none)" ]; then
        ...
        # Refresh once if candidate disappeared.
    ...
    if [ -z "$candidate" ] ||
       [ "$candidate" = "(none)" ]; then
        ...
        fatal_error \
            ...
            "Run 'pkg update' and check the current Termux repositories. If the package is obsolete, remove it from the requested package list."
    fi
    ...
    while [ "$attempt" -le "$max_attempts" ]; do
        ...
        show_progress \
            "Downloading $pkg (attempt $attempt/$max_attempts)"
        ...
        # ----------------------------------------------------
        # If download failed, refresh metadata before retry.
        # ----------------------------------------------------
    ...
    fatal_error \
        ...
        "The Termux package '$pkg' could not be downloaded after multiple attempts. The package metadata was refreshed and the download was retried." \
        "Check the Internet connection, run 'pkg update', and run the builder again. Existing valid packages in offline_packages will be reused."
}
...
prepare_libacl() {
    ...
    if [ -z "$libacl_deb" ]; then
        ...
        # Make sure repository metadata is current.
...
# ============================================================
#
# The generated installer keeps the v3.24 installer behavior.
# ============================================================
...
show_progress \
    "Running offline dependency repair"
