...
# FIXES v3.30
# ------------------------------------------------------------
# • Explicit apt:/pip: package source prefixes
# • PyPI override list so qrcode is never misclassified as APT
# • Package source normalization throughout dependency/download logic
#
# FIXES v3.29
# ------------------------------------------------------------
...
# ============================================================
# PACKAGE SOURCE / TYPE RESOLUTION
# ============================================================
#
# Supported package sources:
#
#   apt:PACKAGE   -> Termux APT / DEB
#   pip:PACKAGE   -> PyPI / wheel / sdist
#   PACKAGE       -> automatic resolution
#
# IMPORTANT:
# Some names can exist in both APT metadata and PyPI.
# The builder therefore has an explicit PyPI override list.
# qrcode is intentionally PyPI for this offline builder.
#
# ============================================================

FORCE_PYPI_PACKAGES="qrcode"
...
is_forced_pypi() {
    ...
    for item in $FORCE_PYPI_PACKAGES; do
        [ "$pkg" = "$item" ] && return 0
    done
...
is_apt_package() {
    ...
    # Explicit pip: always wins.
    if is_explicit_pypi "$spec"; then
        return 1
    fi

    # Explicit apt: always uses APT.
    if is_explicit_apt "$spec"; then
        candidate="$(
    ...
    # Known PyPI override must never be classified as APT.
    if is_forced_pypi "$pkg"; then
        return 1
    fi
...
# qrcode is bundled from PyPI, not Termux APT.
FORCE_PYPI_PACKAGES="qrcode"
...
for spec in $TO_OFFLINE; do

    pkg="$(package_name "$spec")"

    if is_apt_package "$spec"; then
